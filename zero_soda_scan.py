...
_FAQ = [
    ("티어는 어떤 기준으로 나눈 건가요?",
     ["맛, 가격, 인기는 평가에 반영하지 않았습니다. 피어리뷰 연구에서 보고된 "
      "대사적 영향을 기준으로만 분류했습니다. 구체적으로는 열량이 얼마나 되는지, "
      "혈당과 인슐린에 영향을 준다는 근거가 있는지, 그리고 그 근거가 얼마나 "
      "확실한지를 살펴봤습니다.",
      "따라서 맛이 좋은 제품이 낮은 티어를 받거나, 상대적으로 덜 팔리는 제품이 "
      "높은 티어를 받을 수도 있습니다.",
      "이 평가는 정부 기관의 공식 평가가 아니라, 관련 연구를 바탕으로 이 "
      "프로젝트에서 해석한 결과입니다. 의학적 조언이나 진단을 위한 자료는 "
      "아닙니다."],
     "tier-legend"),
    ("제로 음료 중 가장 나은 감미료는 무엇인가요?",
     ["알룰로스와 타가토스입니다. 0.2~0.4 kcal/g 이고 식후 혈당을 오히려 낮춘다는 "
      "메타분석 결과가 있어 이 리포트에서 S 티어입니다. 다만 국내 탄산음료에서 "
      "알룰로스만 쓰는 제품은 드물고 대개 다른 감미료와 섞여 최종 티어가 "
      "내려갑니다."]),
    ("코카콜라 제로에는 어떤 감미료가 들어가나요?",
     ["아스파탐과 아세설팜칼륨입니다. 식약처 신고 원재료에는 '식품첨가물혼합제제'로만 "
      "적혀 있어 감미료가 보이지 않아, 판매처의 상품정보제공 고시 표시사항으로 "
      "확인해 출처와 함께 표시했습니다."]),
    ("제로라고 적혀 있으면 칼로리가 정말 0인가요?",
     ["아닙니다. 제로칼로리 표기 기준은 100mL당 4kcal 미만이라 소량의 열량과 당류가 "
      "있어도 적법하게 '제로'를 붙일 수 있습니다. 이 리포트는 제로를 표방하면서 신고 "
      "원재료에 당류가 있는 제품을 F 티어로 따로 표시합니다."]),
    ("제가 마시는 음료가 대부분 B~C 티어인데 걱정해야 하나요?",
     ["B~C 티어라고 해서 크게 걱정할 필요는 없습니다. 국내에서 판매되는 제로 음료의 "
      "상당수가 이 범위에 해당합니다.",
      "이 티어는 설탕 음료와 비교한 점수가 아니라, 제로 음료끼리 비교한 결과입니다. "
      "같은 제품의 일반판은 대부분 F 티어이기 때문에, B~C 티어 제로 음료를 선택하는 "
      "것만으로도 당류와 열량을 크게 줄이는 선택입니다.",
      "또한 B·C 티어에서도 '일부 대사적 신호가 관찰되었다'는 수준이지, 해당 음료가 "
      "건강에 해롭다는 인과관계가 확정됐다는 연구결과는 아닙니다.",
      "그래도 리스크를 최대한 낮추고 싶다면 S·A 티어 제품을 선택할 수 있지만, 현재 "
      "국내에서 선택할 수 있는 제품은 많지 않습니다."]),
    ("에리스리톨은 피해야 하나요?",
     ["혈당에는 무해하지만 혈소판 반응성·심혈관 사건과 연관된 관찰연구 신호가 있어 이 "
      "리포트에서 C 티어입니다. 인과관계는 확정되지 않았습니다. 스테비아 제품은 대부분 "
...
def write_html(records, meta, meta_info, path):
    ...
    # 리포트에는 티어 기준 패널이 이미 위에 있으므로 FAQ 안에 또 넣지 않는다.
    faq_html = ('<section class="faq">\n<h2>자주 묻는 질문</h2>\n'
                + faq_details_html(_FAQ, with_extra=False) + "\n</section>")
...
def seo_landing_specs(records):
    ...
    def negative(slug, subject, clear, murky, why, extra=""):
        ...
        return (
            ...
            f"신고 원재료가 투명하게 공개된 제품 중 {ga} 없는 것은 "
    ...
    return [
        ("allulose.html",
         ...
         f"수집한 {n}개 제품 중 알룰로스 계열(알룰로스·타가토스)을 신고 "
        ...
        ("fake-zero.html",
         ...
         f"제로를 표기한 제품 가운데 신고 원재료에 당류가 들어간 것은 "
        ...
        ("hidden-zero.html",
         ...
         f"제품명에 제로 표기가 없으면서 100mL당 4kcal 미만인 제품이 "
...
.faq-legend summary{cursor:pointer;color:var(--accent);font-weight:600}
.faq-legend .legend{margin:9px 0 0;background:var(--bg);border:1px solid var(--border);
                    padding:2px 12px}
.faq-legend .tier-row{display:grid;
  grid-template-columns:28px 76px minmax(140px,1fr) minmax(180px,1.6fr);
  gap:9px;align-items:start;padding:8px 0;border-top:1px solid var(--hair)}
.faq-legend .tier-row:first-child{border-top:0}
.faq-legend .tier-row b{font-size:12px}
.faq-legend .tier-ing{font-size:11.5px;color:var(--text)}
.faq-legend .tier-why{font-size:11.5px;color:var(--muted);line-height:1.5}
.faq-legend .tier-note{font-size:11px;color:var(--muted);line-height:1.6;
                       padding:9px 0 2px;border-top:1px solid var(--hair)}
@media(max-width:700px){
  .faq-legend .tier-row{grid-template-columns:28px 1fr;gap:3px 9px}
  .faq-legend .tier-ing,.faq-legend .tier-why{grid-column:2}
}
...
.faq p{margin:9px 0 0;color:var(--muted);font-size:13px;line-height:1.7}
.faq details>p:last-of-type{margin-bottom:2px}
"""
...
# _FAQ 항목은 (질문, [문단…]) 또는 (질문, [문단…], 부가블록키) 다.
# 부가 블록(티어 기준표 등)은 화면에만 싣고 LD 답변에는 넣지 않는다 - LD 는
# 가시 텍스트의 부분집합이면 되고, 표까지 밀어 넣으면 인용 문장이 망가진다.
_FAQ_EXTRA = {"tier-legend": lambda: (
    '<details class="faq-legend"><summary>티어 기준표 보기</summary>'
    '<div class="legend">' + tier_legend_html() + "</div></details>")}


def faq_pairs(items):
    """(질문, 답변 본문) 쌍. 답변은 문단을 이어 붙인 순수 텍스트다."""
    return [(q, " ".join(paras)) for q, paras, *_ in items]


def faq_details_html(items, with_extra=True):
    """<details> 아코디언. 문단은 <p> 로 나누고 부가 블록은 그 아래에 둔다."""
    out = []
    for q, paras, *rest in items:
        body = "".join(f"<p>{x}</p>" for x in paras)
        key = rest[0] if rest else None
        if with_extra and key:
            body += _FAQ_EXTRA[key]()
        out.append(f"<details><summary>{q}</summary>{body}</details>")
    return "\n".join(out)
...
def landing_page(records, lastmod, stats):
    ...
    faq_html = faq_details_html(_FAQ)
