...
| 제로 표기 없는 일반 음료 제외 | −1,064건 |
| 제품 단위로 통합한 최종 레코드 | **635개** |
| 열량·당류 조인 성공 | 428개 (67.4%) |
| 제품명에 제로 표기 | 325개 |
| └ 그중 **원재료에 당류가 있는 제품** | **16개** |
| 배합 변경 이력이 확인된 제품 | 138개 |
...
```bash
python -m unittest test_classify -v    # 28 tests — 분류 로직 회귀 검증
```
...
| `test_classify.py`         | 분류/알코올판정/파생플래그 테스트 28개                |
...
`감미료미표기` `이력행수` `배합변경` `티어불일치` `원재료전문` `제로표기` `제로사칭` `카페인` `아스파탐` `일반판` `일반판티어`
