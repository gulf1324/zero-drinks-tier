...
### 성분 근거는 '품목보고번호 일치'로 판단한다 - 2026-09-24 사용자 결정

원재료(`원재료` 필드)를 채우는 순서는 이렇다.

1. **식약처 신고 원재료 (C002)** — 항상 먼저다. 감미료를 하나라도 짚었으면 끝이다.
2. C002 가 감미료를 못 짚었을 때(`?`/`무감미료`)만 **라벨 표시 원재료**를 쓴다.
   **출처가 공식 페이지든 판매처든 상관없다. 라벨에 인쇄된 품목보고번호가 그
   제품의 C002 번호와 일치하면 검증된 것으로 본다.** 일치하지 않거나 번호가
   없으면 쓰지 않는다.

이 조건 하나가 아래를 모두 걸러낸다.

- 오독 — 잘못 읽은 번호는 C002 에 없다.
- 해외 공식 사이트 — 국내 신고번호가 없다. `gorillaenergy.com` 은 번호 없이 C002 에
  없는 `ginseng` 을 적고 있었다 (2026-09-24).
- 자매 제품 — 번호가 다르다. `클룹 제로소다 소다맛` 라벨의 '당류 0g'을 `사이다`에
  옮길 수 없는 이유다.
- 블로그·나무위키·영양앱·검색 결과 요약 — 품목보고번호가 인쇄된 라벨이 아니다.

웹 검색은 라벨이 실린 페이지를 **찾는 수단**이다. 근거는 그 페이지의 라벨 이미지다.

찾지 못하면 비워 두고 다음 갱신 때 다시 확인한다. 미해결 목록은 `docs/LABELS.md`.
기존 수동 등록 55건(제타·컬리·SSG·올리브영·쿠팡·옥션·농협몰)은 전부 번호를 대조해
통과한 것이라 이 규칙에 맞는다.
