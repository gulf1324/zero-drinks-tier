...
| 후보 | 판정 | 근거 |
|---|---|---|
| `I2570` 유통바코드 | **최유력**. `PRDT_NM`(유통 제품명)·`BRCD_NO`·`PRDLST_REPORT_NO` | 서비스 실재 확인. **개별 활용신청 필요** (현재 키는 C002만 승인) |
| `C005` 바코드연계제품정보 | 유력. `PRDLST_NM`·`BAR_CD`·`END_DT`·`PRDLST_REPORT_NO` | 같음. 등록명 복제일 가능성 있어 실데이터 확인 필요 |
| `I2852` 생산중단제품정보 | 보조. `END_DT`(생산중단일)로 단종 제품 제거 가능 | 같음 |
